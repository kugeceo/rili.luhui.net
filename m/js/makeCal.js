...
var calander = {
		...
		for ( var i = 0; i < rows; i++)
		{
			...
			for ( var j = 0; j < 7; j++)
			{
				if(calData[i][j]!=null){
					...
					var aDay = "";
					if(is5Row)
						aDay = "<td i="+i+" j="+j+" class='block block5 "+tdclass+isClickBlock+"' style='height:"+(1/rows).toPercent()+";'>";
					else
						aDay = "<td i="+i+" j="+j+" class='block "+tdclass+isClickBlock+"' style='height:"+(1/rows).toPercent()+";'>";
					...
					if(restOrWork!=''){
						if(restOrWork=="班"){
							aDay += "<div class='jprestWork jprestWorkw'><img src='img/ban.png' border='0' width='13' height='13' /></div>";
						}else{
							aDay += "<div class='jprestWork'><img src='img/jia.png' border='0' width='13' height='13' /></div>";
						}
					}else{
						//aDay += "<div class='jprestWork'>&nbsp;</div>";
					}
					aDay += "<div style='display:inline-block;position:absolute;top:50%;width:100%;margin-top:-30px;left:0;'>";
					if(calData[i][j].weekend&&!(calData[i][j].after||calData[i][j].before)){
						...
						if(calData[i][j].after||calData[i][j].before){
							aDay += "<div class='"+numtype+"' style='color:#aaa;'>"+dateDay+"</div>";
						}else{
							aDay += "<div class='"+numtype+"'>"+dateDay+"</div>";
						}
					}
	...
	initAction:function()
	{
		...
		next_button_a.addEventListener('touchstart', function(){
			$(this).css('opacity','0.5');
		});
		next_button_a.addEventListener('touchend', function(){
			$(this).css('opacity','1');
		...
		prev_button_a.addEventListener('touchstart', function(){
			$(this).css('opacity','0.5');
		});
		prev_button_a.addEventListener('touchend', function(){
			$(this).css('opacity','1');
	   ...
	   today_button.addEventListener('touchstart', function(){
			$(this).css('opacity','0.5');
		});
	   today_button.addEventListener('touchend', function(){
			$(this).css('opacity','1');
		...
		$('#yearValue').text(currentDate.getFullYear());
		$('#monthValue').text(currentDate.getMonth()+1);
	...
	makeAction:function(){
		var mainCal = document.getElementById('mainCal');
		mainCal.addEventListener('touchstart', function(event){
			$(event.target).parent().parent().parent().css("background","#f5f5f5");
		});
		mainCal.addEventListener('touchend', function(event){
	    	$(event.target).parent().parent().parent().css("background","#fff");
	   // });
		
		/*$('.block').unbind();
		$('.block').bind('click', function(e){*/
			$('.block').removeClass("block_click");
			$(event.target).parent().parent().parent().addClass("block_click");
			var real_show_month = madeRiliDate.getMonth();
			ele = $(event.target);
			while(1)
			{
			...
			$("#mothDayBody").show();
			$("#mothDayBody").css('left',"100%");
			$("#body").css('left','0');
			$("#mothDayBody").animate({left:"0"},20);
	...
	makeHuangli:function(date)
	{
		...
		$('#chinaWeek').text(currentDate.format("w"));
		$('#monthDayValue').text(currentDate.getDate());
		$('#monthMValue').text(currentDate.getMonth()+1);
		$('#yearDayValue').text(currentDate.format("yyyy"));
